...
file="$(tail -n +34 $1)"
sampleid=$2
echo "Position  REF	ALT	LABEL	Description	BaseSupport_QP	AlleleSupport_AF" > "$sampleid".minority_alleles_report.tech
...
then
echo "$POS	$REF	$base	$LABEL	$tag	$reads	$afnumb" >> "$sampleid".minority_alleles_report.tech
